...
// solution 1
// initially, we check if the basket key exists and has a value in the localstorage
if(localStorage.getItem('basket')) {
    var basketJSON = localStorage.getItem('basket');
    basket = JSON.parse(basketJSON);

    // now we have an object that we can use to create the item list
    // in HTML
    for(var i=0;i<basket.length;i++) {
        var newListItem = document.createElement('li');
        newListItem.innerHTML = basket[i].name;
